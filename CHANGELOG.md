...
- Added forward-compatible persisted runtime-mode decoding so rollback to an older build falls back to `approval-required` (least privilege), never `full-access`, instead of failing startup.
