...
#   1. Refuses to run against a dirty working tree or a non-fork branch.
#   2. Fetches the `upstream` remote (tags + branches).
#   3. Picks a rebase target: the highest non-nightly `vX.Y.Z` tag reachable
#      from `upstream/main`, falling back to `upstream/main` itself when no
#      such tag exists.
...
#      - On success: runs `pnpm run typecheck` and the server test suite
#        (`pnpm --filter t3 run test`) and reports the result.
#
# Usage: scripts/rebase-upstream.sh
#
set -euo pipefail
...
if [[ -n "$(git status --porcelain)" ]]; then
  fail "Working tree is not clean. Commit or stash changes before rebasing."
fi
...
log "Running typecheck (pnpm run typecheck)..."
...
log "Running server tests (pnpm --filter t3 run test)..."
if ! pnpm --filter t3 run test; then
  fail "Server tests failed after a clean rebase. The rebase itself succeeded (see $BACKUP_BRANCH for the pre-rebase state if you need to compare); investigate before merging or pushing."
fi

log "Rebase onto $REBASE_TARGET verified: typecheck and server tests are green."
