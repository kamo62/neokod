...
Stages 1-2 remove Neokod's mobile/marketing products, SSH backend transport,
Tailscale integration, LAN advertising, and selectable server binding. The desktop primary, standalone
...
until Stage 5. Stage 3 removes the hosted service deployment and outbound mobile-activity publisher;
the pure shared agent-awareness projection and local browser notifications remain. Stage 4 removes
the hosted application and pairing surface, identity-provider integration, and remote connection
client/contracts. The temporary local session/auth control plane and the `@t3tools/*` namespace
remain for Stages 5 and 6 respectively.
...
| `apps/server/src/ws.ts`                                                                                                                                       | One import, one `RPC_REQUIRED_SCOPE` entry (operate scope), one RPC handler delegating to the fork-owned `testManagedClientEvidenceConnection` with the saved settings. Plus three device-login handlers and their operate-scope entries delegating to `GithubDeviceLogin.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Serves the governance test-connection RPC using the established observe/scope wrapper; all logic stays in the fork-owned module.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `packages/client-runtime`, `packages/contracts`, and `packages/shared` remote connection surfaces (local-first 2.0.0) | Removed the remote connection target/client/contracts, token persistence, discovery, tracing, exports, and tests; authorization is bearer-only. | Retain only the temporary local/WSL bearer boundary until Stage 5. |
...
| `apps/web/src/branding.ts`, `apps/web/index.html`, `apps/desktop/src/**`, `apps/web/src/**`                                                 | Replaces user-visible T3 Code product copy with Neokod in active desktop/web surfaces.                                                                                                                                            | Internal `T3CODE_*` compatibility names stay unchanged to reduce upstream rebase cost; preserve Neokod only for user-facing identity.                    |
