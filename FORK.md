...
| `apps/web/src/components/ChatView.tsx` (subagents)                         | One import + one `rightPanelContent` branch rendering `<SubagentsPanel key={activeThreadId} activities={threadActivities} …>`, mirroring the `plan` branch. Plus a guarded auto-open effect: the first `task.started` in a thread opens the subagents surface, but only when the right panel is closed (never steals an open surface) and only once per thread (`autoOpenedSubagentsRef`), mirroring the plan-sidebar auto-open precedent. The `key` resets per-thread panel selection so a selected worker never leaks across threads.                                                                                                                                                                                                                                                                                                                                                           |
