...
Neokod connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

Neokod works with the platforms your team already uses:
...
- Neokod can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Neokod:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Neokod and verify GitHub shows as authenticated
...
2. Add these environment variables to the environment running Neokod:
   ```bash
   export NEOKOD_BITBUCKET_EMAIL="you@example.com"
   export NEOKOD_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Neokod and verify the connection in **Source Control settings**
...
**Git is required** – Neokod uses Git for all local operations. Ensure `git` is installed on your server.
