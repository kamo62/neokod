...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.kamo62.neokod.dev");
      assert.equal(environment.linuxWmClass, "neokod-dev");
      assert.deepEqual(
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.kamo62.neokod.dev.local");
    }),
  );
