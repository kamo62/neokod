...
function input(
  environmentId: typeof ENV_A | typeof ENV_B,
  nowMs: number,
  overrides: Partial<EnvironmentActivityInput> = {},
...
function observe(state: ActivityObservationState, value: EnvironmentActivityInput) {
  return reduceEnvironmentActivityObservation(state, value);
}
...
describe("activity notifications occurrence reducer", () => {
  it("A-U01 baselines cached, synchronizing, and staggered environments", () => {
    ...
    let result = observe(
      ...
      input(ENV_A, 0, {
        shellStatus: "cached",
        threads: [thread({ latestTurn: completed("old") })],
      }),
    );
    ...
    result = observe(
      result.state,
      input(ENV_A, 2, { threads: [thread({ latestTurn: completed("old") })] }),
    );
    expect(result.occurrences).toEqual([]);
    result = observe(
      result.state,
      input(ENV_B, 3, { shellStatus: "cached", threads: [thread({ latestTurn: failed("old") })] }),
    );
    expect(
      observe(result.state, input(ENV_B, 4, { threads: [thread({ latestTurn: failed("old") })] }))
        .occurrences,
    ).toEqual([]);
    result = observe(
      result.state,
      input(ENV_A, 5, { threads: [thread({ latestTurn: completed("new") })] }),
    );
  ...
  it("A-U02 emits a coalesced-render completion once and settles repeat observation", () => {
    let state = observe(
      createActivityObservationState(),
      input(ENV_A, 0, { threads: [thread({ latestTurn: completed("old") })] }),
    ).state;
    const result = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ latestTurn: completed("turn-1") })] }),
    );
    expect(result.occurrences).toMatchObject([
      { kind: "agent-completed", reliability: "exact", turnId: "turn-1" },
    ]);
    const settled = flush(result.state, 251).state;
    expect(
      observe(settled, input(ENV_A, 2, { threads: [thread({ latestTurn: completed("turn-1") })] }))
        .occurrences,
    ).toEqual([]);
  });
  ...
  it("A-U03 settles after attention and A-U04 settles a same-generation active turn", () => {
    ...
    const attention = observe(
      state,
      input(ENV_A, 1, {
        threads: [thread({ session: running("turn-1"), hasPendingApprovals: true })],
      }),
    );
    ...
    let result = observe(
      state,
      input(ENV_A, 2, { threads: [thread({ latestTurn: completed("turn-1") })] }),
    );
    expect(result.occurrences).toMatchObject([{ kind: "agent-completed", turnId: "turn-1" }]);
    expect(flush(result.state, 252).delivered).toMatchObject([
    ...
    state = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ session: running("turn-2") })] }),
    ).state;
  ...
  it("A-U05 keeps ready-at-birth, title-only, and ID-less transitions silent", () => {
    let state = observe(
      createActivityObservationState(),
      input(ENV_A, 0, { threads: [thread({ session: ready(null) })] }),
    ).state;
    expect(
      observe(
        state,
        input(ENV_A, 1, { threads: [thread({ title: "Renamed", session: ready(null) })] }),
      ).occurrences,
    ).toEqual([]);
    state = observe(createActivityObservationState(), input(ENV_A, 0)).state;
    state = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ session: starting(null) })] }),
    ).state;
    expect(
      observe(state, input(ENV_A, 2, { threads: [thread({ session: ready(null) })] })).occurrences,
    ).toEqual([]);
  });
  ...
  it("is edge-triggered for ID-less failures", () => {
    ...
    expect(result.occurrences).toMatchObject([
      { kind: "agent-failed", reliability: "best-effort", ordinal: 1 },
    ]);
    result = observe(
      result.state,
      input(ENV_A, 2, { threads: [thread({ session: error(null) })] }),
    );
    expect(result.occurrences).toEqual([]);
  });
  ...
  it("consumes cached active turns that settle on their first live terminal snapshot", () => {
    let state = observe(
      createActivityObservationState(),
      input(ENV_A, 0, { shellStatus: "cached", threads: [thread({ session: running("old") })] }),
    ).state;
    ...
    const firstLive = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ latestTurn: failed("old") })] }),
    );
    expect(firstLive.occurrences).toEqual([]);
    expect(
      observe(firstLive.state, input(ENV_A, 2, { threads: [thread({ session: ready(null) })] }))
        .occurrences,
    ).toEqual([]);
  });
  ...
  it("preserves a live completion across cached and synchronizing snapshots", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ session: running("turn-1") })] }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 2, {
        shellStatus: "cached",
        threads: [thread({ latestTurn: completed("turn-1") })],
      }),
    ).state;
    state = observe(
      state,
    ...
    expect(
      observe(state, input(ENV_A, 4, { threads: [thread({ latestTurn: completed("turn-1") })] }))
        .occurrences,
  ...
  it("preserves a live failure across cached and synchronizing snapshots", () => {
    ...
    state = observe(
      ...
      input(ENV_A, 2, {
        shellStatus: "cached",
        threads: [thread({ latestTurn: failed("turn-1") })],
      }),
    ).state;
    state = observe(
      state,
    ...
    expect(
      observe(state, input(ENV_A, 4, { threads: [thread({ latestTurn: failed("turn-1") })] }))
        .occurrences,
  ...
  it("settles the original live completion when the replacement fails offline", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ session: running("turn-a") })] }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 2, {
        shellStatus: "cached",
        threads: [thread({ session: ready(null), latestTurn: completed("turn-a") })],
      }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 3, {
        shellStatus: "synchronizing",
        threads: [thread({ session: running("turn-b"), latestTurn: completed("turn-a") })],
      }),
    ).state;
    expect(
      observe(
        state,
        input(ENV_A, 4, {
          threads: [thread({ session: error("turn-b"), latestTurn: failed("turn-b") })],
        }),
      ).occurrences,
  ...
  it("settles the original live failure when the replacement completes offline", () => {
    ...
    state = observe(
      ...
      input(ENV_A, 2, {
        shellStatus: "cached",
        threads: [thread({ session: error(null), latestTurn: failed("turn-a") })],
      }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 3, {
        shellStatus: "synchronizing",
        threads: [thread({ session: running("turn-b"), latestTurn: failed("turn-a") })],
      }),
    ).state;
    expect(
      observe(
        state,
        input(ENV_A, 4, {
          threads: [thread({ session: running("turn-b"), latestTurn: completed("turn-b") })],
        }),
      ).occurrences,
  ...
  it("uses a scope window so failure at t249 wins approval at t0", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 0, { threads: [thread({ hasPendingApprovals: true })] }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 249, {
        threads: [thread({ hasPendingApprovals: true, session: error("turn-1") })],
      }),
    ).state;
    ...
    const settled = settleActivityOccurrence(
      first.state,
      activityOccurrenceKey(first.occurrence!),
      "delivered",
    );
    expect(flush(settled, 250).delivered).toMatchObject([
      { kind: "approval-needed", reliability: "best-effort" },
    ]);
  });
  ...
  it("A-U07 retries the failed approval before delivering completion", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ hasPendingApprovals: true })] }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 2, {
        threads: [thread({ hasPendingApprovals: true, latestTurn: completed("turn-1") })],
      }),
    ).state;
    const first = flushNextActivityOccurrence(state, 251);
    const failed = settleActivityOccurrence(
      first.state,
      activityOccurrenceKey(first.occurrence!),
      "failed",
    );
    ...
    const delivered = settleActivityOccurrence(
      retry.state,
      activityOccurrenceKey(retry.occurrence!),
      "delivered",
    );
  ...
  it("retains attention ordinals through tombstone reappearance false then a fresh rise", () => {
    ...
    const result = observe(
      state,
      input(ENV_A, 4, { threads: [thread({ hasPendingApprovals: true })] }),
    );
    expect(result.occurrences).toMatchObject([
      { kind: "approval-needed", reliability: "best-effort", ordinal: 2 },
    ]);
  });
  ...
  it("A-U08 suppresses a pending-true tombstone reappearance", () => {
    ...
    expect(
      observe(
        state,
        input(ENV_A, 3, {
          threads: [thread({ hasPendingApprovals: true, updatedAt: "2026-07-12T00:00:03.000Z" })],
        }),
      ).occurrences,
    ).toEqual([]);
  });
  ...
  it("A-U09 baselines pending input on reconnect", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 1, { generation: 2, threads: [thread({ hasPendingUserInput: true })] }),
    ).state;
    expect(
      observe(
        state,
        input(ENV_A, 2, { generation: 2, threads: [thread({ hasPendingUserInput: true })] }),
      ).occurrences,
    ).toEqual([]);
  });
  ...
  it("A-U10 consumes disabled queued and in-flight occurrences before re-enable", () => {
    ...
    state = observe(state, input(ENV_A, 3, { threads: [thread()] })).state;
    expect(
      observe(state, input(ENV_A, 4, { threads: [thread({ hasPendingApprovals: true })] }))
        .occurrences,
    ).toMatchObject([{ kind: "approval-needed", ordinal: 2 }]);
  });
  ...
  it("uses raw latest-turn error despite pending input", () => {
    ...
    const result = observe(
      state,
      input(ENV_A, 1, {
        threads: [thread({ latestTurn: failed("turn-1"), hasPendingUserInput: true })],
      }),
    );
    expect(result.occurrences).toMatchObject([
  ...
  it("treats interrupted turns with completedAt as exact completion and ignores timestamp-only changes", () => {
    ...
    let result = observe(
      state,
      input(ENV_A, 1, { threads: [thread({ latestTurn: interrupted("turn-1", NOW) })] }),
    );
    expect(result.occurrences).toMatchObject([
      { kind: "agent-completed", reliability: "exact", turnId: "turn-1" },
    ]);
    ...
    result = observe(
      ...
      input(ENV_A, 2, {
        threads: [
          thread({
            latestTurn: interrupted("turn-1", "2026-07-12T00:00:02.000Z"),
  ...
  it("increments terminal episode only on a new running episode", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 1, { terminals: [terminal({ hasRunningSubprocess: true })] }),
    ).state;
    const result = observe(
      state,
      input(ENV_A, 2, { terminals: [terminal({ hasRunningSubprocess: false })] }),
    );
    expect(result.occurrences).toMatchObject([
      { kind: "terminal-completed", reliability: "best-effort", ordinal: 1 },
    ]);
  });
  ...
  it("A-U11 suppresses terminal completion on reconnect but emits a fresh edge", () => {
    ...
    state = observe(
      state,
      input(ENV_A, 2, { generation: 2, terminals: [terminal({ hasRunningSubprocess: false })] }),
    ).state;
    const freshRunning = observe(
      state,
      input(ENV_A, 3, { generation: 2, terminals: [terminal({ hasRunningSubprocess: true })] }),
    );
    ...
    expect(
      observe(
        state,
        input(ENV_A, 4, { generation: 2, terminals: [terminal({ hasRunningSubprocess: false })] }),
      ).occurrences,
    ).toMatchObject([{ kind: "terminal-completed" }]);
  });

  it("baselines delayed initial terminal metadata and replacement snapshots", () => {
    let state = observe(
      createActivityObservationState(),
      input(ENV_A, 0, { terminalMetadataReady: false }),
    ).state;
    state = observe(
      state,
      input(ENV_A, 1, {
  ...
  it("A-U12 bounds delivered keys and tombstones, and scopes identical ids independently", () => {
    ...
    for (let index = 0; index < 513; index += 1)
      state = enqueueActivityOccurrences(state, [
        occurrence(ENV_A, `thread-${index}`, `turn-${index}`),
      ]);
    state = flush(state, 250).state;
    expect(state.deliveredKeys.size).toBe(512);
    state = enqueueActivityOccurrences(state, [
      occurrence(ENV_A, "same-thread", "same-turn"),
      occurrence(ENV_B, "same-thread", "same-turn"),
    ]);
    expect(flush(state, 250).delivered).toHaveLength(2);
    expect(scopedThreadKey(scopeThreadRef(ENV_A, "same-thread" as never))).not.toBe(
      scopedThreadKey(scopeThreadRef(ENV_B, "same-thread" as never)),
    );
    state = observe(createActivityObservationState(), input(ENV_A, 0)).state;
    for (let index = 0; index < 513; index += 1) {
      state = observe(
        state,
        input(ENV_A, index + 1, { threads: [thread({ id: `tombstone-${index}` as never })] }),
      ).state;
...
function running(turnId: string | null) {
  return session("running", turnId);
}
function ready(turnId: string | null) {
  return session("ready", turnId);
}
function starting(turnId: string | null) {
  return session("starting", turnId);
}
function error(turnId: string | null) {
  return session("error", turnId);
}
function completed(turnId: string): OrchestrationLatestTurn {
  return {
...
function failed(turnId: string): OrchestrationLatestTurn {
  return { ...completed(turnId), state: "error", completedAt: null };
}
function interrupted(turnId: string, completedAt: string | null): OrchestrationLatestTurn {
  return { ...completed(turnId), state: "interrupted", completedAt };
}
function occurrence(environmentId: typeof ENV_A | typeof ENV_B, threadId: string, turnId: string) {
  return {
