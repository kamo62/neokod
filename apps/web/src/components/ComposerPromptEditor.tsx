...
import { type ServerProviderSkill } from "@neokod/contracts";
      ...
      <div className="relative">
        <PlainTextPlugin
          contentEditable={
            <ContentEditable
              className={cn(
                "block max-h-50 min-h-17.5 w-full overflow-y-auto whitespace-pre-wrap wrap-break-word bg-transparent text-[16px] sm:text-chat leading-[var(--line-height-chat)] text-text-primary focus:outline-none",
                className,
              )}
          ...
          placeholder={
            terminalContexts.length > 0 ? null : (
              <div className="pointer-events-none absolute inset-0 text-[16px] sm:text-chat leading-[var(--line-height-chat)] text-text-tertiary">
                {placeholder}
              </div>
            )
          }
...
export function ComposerPromptEditor({
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "neokod-composer-editor",
      editable: true,
