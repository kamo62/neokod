  ...
  const startSession: CodexAdapterShape["startSession"] = (input) =>
    Effect.scoped(
      Effect.gen(function* () {
          ...
          ...(mcpSession
            ? {
                environment: {
                  ...
                  NEOKOD_MCP_BEARER_TOKEN: mcpSession.authorizationHeader.replace(/^Bearer\s+/, ""),
                },
                appServerArgs: [
                  "-c",
                  `mcp_servers.neokod.url=${mcpSession.endpoint}`,
                  "-c",
                  'mcp_servers.neokod.bearer_token_env_var="NEOKOD_MCP_BEARER_TOKEN"',
                ],
              }
            : {}),
        };
        ...
        // into `sessionScope` keeps provider events flowing even if the
