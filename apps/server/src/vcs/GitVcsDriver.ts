...
export class GitVcsDriver extends Context.Service<
  GitVcsDriver,
  {
    ...
    ) => Effect.Effect<ReviewChangedFilesResult, GitCommandError>;
    readonly getFileDiff: (
      input: ReviewFileDiffInput,
    ) => Effect.Effect<ReviewFileDiffResult, GitCommandError>;
  ...
  const checkpoints: VcsDriver.VcsCheckpointOps = {
    captureCheckpoint: Effect.fn("GitVcsDriver.checkpoints.captureCheckpoint")(function* (input) {
      ...
      const tempIndexPath = path.join(
        gitCommonDir,
        `neokod-checkpoint-index-${NodeCrypto.randomUUID()}`,
      );
      const commitEnv: NodeJS.ProcessEnv = {
        ...process.env,
        GIT_INDEX_FILE: tempIndexPath,
        GIT_AUTHOR_NAME: "Neokod",
        GIT_AUTHOR_EMAIL: "neokod@users.noreply.github.com",
        GIT_COMMITTER_NAME: "Neokod",
        GIT_COMMITTER_EMAIL: "neokod@users.noreply.github.com",
      };
      ...
      yield* Effect.gen(function* () {
        ...
        const message = `neokod checkpoint ref=${input.checkpointRef}`;
